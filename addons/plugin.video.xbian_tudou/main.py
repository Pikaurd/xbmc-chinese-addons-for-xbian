...
def seriesList(name,url,thumb,res):
    ...
    if vid_search:
        ...
        tudou_response = GetHttpData(playlist_url)
        tudou_json = json.loads(tudou_response)
        playlist_length = len(tudou_json.get('items'))
        for item in tudou_json.get('items'):
            episode = item.get('episode')
            title = item.get('title').encode('utf8')
            item_url = item.get('itemPlayUrl')
            p_thumb = item.get('picUrl')

            # test area
            urllib.quote_plus(title)
            urllib.quote_plus(item_url)
            urllib.quote_plus(p_thumb)
            # // end test

            # for xbmc list
            li = xbmcgui.ListItem(title, iconImage = '', thumbnailImage = p_thumb)
            u = sys.argv[0] + "?mode=3&name=" + urllib.quote_plus(title) + "&url=" + urllib.quote_plus(item_url)+ "&thumb=" + urllib.quote_plus(p_thumb)+"&res="+str(res)
            xbmcplugin.addDirectoryItem(int(sys.argv[1]), u, li, False, playlist_length)

        xbmcplugin.setContent(int(sys.argv[1]), 'episodes')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    else:
        dialog = xbmcgui.Dialog()
        ok = dialog.ok(__addonname__, '解析节目列表发生错误')
...
#        if match:
#            prepare_video(name,match[0],thumb,res)
#        else:
#            dialog = xbmcgui.Dialog()
...
def PlayYouku(name,url,thumb,res):
    ...
    if match:
        ...
        for i, u in enumerate(urls):
            title = '第{0}/{1}节'.format(i+1, len(urls))
...
def prepare_video(name,url,thumb,res):
    link = GetHttpData(url)
...
if mode == None:
    ...
    prepare_video(name,url,thumb,res)
elif mode == 4:
