...
def PlayVideo(name,url,thumb,res):
    print('name: {0}\turl: {1}\tthumb: {2}\tres: {3}'.format(name, url, thumb, RES_LIST[res]))
    videoID = re.search('(?<=id_)[\w\d]+', url).group()
    timestamp = int(os.times()[4])
    playLocation = 'http://v.youku.com/player/getM3U8/vid/{0}/type/mp4/ts/{1}/v.m3u8'.format(videoID, timestamp)
    #playLocation = 'http://v.youku.com/player/getM3U8/vid/{0}/type/{1}/ts/{2}/v.m3u8'.format(videoID, RES_LIST[res], timestamp)
    print('Play Location: ' + playLocation)
    ...
    playlist.add(playLocation, listItem)
